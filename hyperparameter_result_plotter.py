...
def plot_tuning_results(show_std=False):
    ...
    tournament_sizes = sorted(
        set(d["tournament_size"] for d in data)
    )
    ...
    for ax, t_size in zip(axes.flat, tournament_sizes):
        ...
        ax.set_xticklabels(
            [f"{m:.0%}" for m in mutation_rates]
        )

        # Y-Achse
        ax.set_yticks(range(len(survivor_rates)))
        ax.set_yticklabels(
            [f"{s:.0%}" for s in survivor_rates]
        )
